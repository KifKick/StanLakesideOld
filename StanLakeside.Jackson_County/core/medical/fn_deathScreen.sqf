...
waitUntil {
	_nearby = 99999;
	{
		if (side _x isEqualTo independent && !(_x getVariable["dead",FALSE]) && _x != player) then
		{
	...
	if (_nearby isEqualTo 99999) then { _medicsNear ctrlSetText format["Nearest: None",_nearby]; }
