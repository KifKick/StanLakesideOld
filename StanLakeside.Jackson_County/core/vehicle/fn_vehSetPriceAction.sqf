...
if(!((getPlayerUID player) isEqualTo _vehOwner)) exitWith { ["Nie mozesz sprzedac pojazdu ktory nie nalezy do Ciebie!", false] spawn domsg; }; //sprawdzamy czy wszystko jest dobrze
if(count _vehData isEqualTo 0) exitWith {["Pojazd nie pochodzi od bazy danych.", false] spawn domsg;}; //sprawdzamy czy jest z garazu
