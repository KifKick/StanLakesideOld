...
switch (playerSide) do
{
	case independent:
	{
		...
		life_actions pushBack (player addAction["<t color = '#d02b2b'>Wyskocz</t>",life_fnc_EmergencyEject,"",0,false,false,"",' (driver vehicle player) isEqualTo player && (vehicle player) isKindOf "Air" ']);
		...
		life_actions pushBack (player addAction["<t color = '#D660D6'>Przenies gracza</t>",life_fnc_dragbody,"",7,false,false,"",' !life_action_inUse && vehicle player isEqualTo player && (cursortarget getVariable["dead",FALSE]) && (player distance cursorTarget < 5) ']);

		life_actions pushBack (player addAction["<t color='#ADFF2F'>Bankomat</t>",life_fnc_atmMenu,"",7,false,false,"",' (cursorTarget getVariable ["ATM_MAN", false]) && cgbankvault animationPhase "d_l_Anim" isEqualTo 0 && (player distance (getMarkerPos "bank_signup")) < 21 ']);
		...
		life_actions pushBack (player addAction["Wstan",life_fnc_electric,"",0,false,false,"",' ( animationState player isEqualTo "BasicDriver" && isNull objectParent player )  ']);
		...
		_cones = nearestObjects[getPos player,["RoadCone_F"],3.5] select 0; !isNil "_cones" && !isNil {(_cones getVariable "roadcone")} && {count (_cones getVariable "roadcone") isEqualTo 1}
		']);
		life_actions pushBack (player addAction["Zbierz pacholki",life_fnc_packupRoadcones,"",0,false,false,"",'
		...
		']);
		life_actions pushBack (player addAction["Podnies pacholek",life_fnc_packupRoadcones,"",0,false,false,"",'
		_cones = nearestObjects[getPos player,["RoadCone_L_F"],3.5] select 0; !isNil "_cones" && !isNil {(_cones getVariable "roadcone")} && {count (_cones getVariable "roadcone") isEqualTo 1}
		']);
		...
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "RoadconeStripB" && (player distance cursorTarget) < 3 ']);
		life_actions pushBack (player addAction["Podnies pacholki",life_fnc_pickupItem,"",0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "RoadconeStrip" && (player distance cursorTarget) < 3 ']);
		life_actions pushBack (player addAction["Podnies swiecacy pacholek",life_fnc_pickupItem,"",0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "RoadconeB" && (player distance cursorTarget) < 3 ']);
		life_actions pushBack (player addAction["Podnies pacholek",life_fnc_pickupItem,"",0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "Roadcone" && (player distance cursorTarget) < 3 ']);
		...
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "RoadBlockWood" && (player distance cursorTarget) < 3 ']);
		life_actions pushBack (player addAction["Podnies blokade drogowa (wzmocniona)",life_fnc_pickupItem,"",0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "RoadBlockConc" && (player distance cursorTarget) < 3 ']);
		life_actions pushBack (player addAction["Pickup RoadBlock (Wreck)",life_fnc_pickupItem,"",0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "RoadBlockRebel" && (player distance cursorTarget) < 3 ']);
	};
	case civilian:
	{
		...
		life_actions pushBack (player addAction[localize "STR_pAct_Breakout",life_fnc_breakout,"",0,false,false,"",' life_breakouton isEqualTo 2 && (!life_action_inUse) ']);
	...
	case west:
	{
		...
		life_actions pushBack (player addAction["<t color = '#ADFF2F'>Przenies gracza</t>",life_fnc_dragbody,"",7,false,false,"",' !isdragging && !life_action_inUse && vehicle player isEqualTo player && (cursortarget getVariable["dead",FALSE]) && (player distance cursorTarget < 5) ']);
		...
		//gatherevidence
		life_actions pushBack (player addAction["Zbierz dowody",life_fnc_gatherEvidence,"",0,false,false,"",' !isNull cursorTarget && player distance cursorTarget < 2.5 && !dialog && typeOf cursorTarget isEqualTo "Land_Suitcase_F"']);
		...
		//electrocute victim
		life_actions pushBack (player addAction["Electrocute Victim",life_fnc_electricchairdo,"",0,false,false,"",' ( animationState cursorTarget isEqualTo "BasicDriver" && vehicle cursorTarget isEqualTo cursorTarget )  ']);
		...
		!isNull cursorTarget && player distance cursorTarget < 3 && isPlayer cursorTarget && (cursorTarget getVariable ["bleedingon", 1]) isEqualTo 2 ']);

		life_actions pushBack (player addAction[localize "STR_pAct_NoStitchPlayer","","",0,false,false,"",'
		!isNull cursorTarget && player distance cursorTarget < 3 && isPlayer cursorTarget && (cursorTarget getVariable ["woundedon", 1]) isEqualTo 2 ']);

		life_actions pushBack (player addAction[localize "STR_pAct_NoSplintPlayer","","",0,false,false,"",'
		!isNull cursorTarget && player distance cursorTarget < 3 && isPlayer cursorTarget && (cursorTarget getVariable ["fractureon", 1]) isEqualTo 2 ']);

		life_actions pushBack (player addAction[localize "STR_pAct_NoEpiPenPlayer","","",0,false,false,"",'
		!isNull cursorTarget && player distance cursorTarget < 3 && isPlayer cursorTarget && (cursorTarget getVariable ["unconciouson", 1]) isEqualTo 2 ']);
		...
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "MDMAu" && (player distance cursorTarget) < 3 && (life_carryWeight + (["MDMAu"] call life_fnc_itemWeight)) <= life_maxWeight ']);
		//Pickup Processed MDMA
		life_actions pushBack
		(player addAction["Pickup Processed MDMA",life_fnc_pickupItem,true,0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "MDMAp" && (player distance cursorTarget) < 3 && (life_carryWeight + (["MDMAp"] call life_fnc_itemWeight)) <= life_maxWeight ']);

		//Pickup Unprocessed Meth
		life_actions pushBack
		(player addAction["Pickup Unprocessed Meth",life_fnc_pickupItem,false,0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "methu" && (player distance cursorTarget) < 3 && (life_carryWeight + (["methu"] call life_fnc_itemWeight)) <= life_maxWeight ']);
		//Pickup Processed Meth
		life_actions pushBack
		(player addAction["Pickup Processed Meth",life_fnc_pickupItem,true,0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "methp" && (player distance cursorTarget) < 3 && (life_carryWeight + (["methp"] call life_fnc_itemWeight)) <= life_maxWeight ']);

		//Pickup Unprocessed Coke
		life_actions pushBack
		(player addAction["Pickup Unprocessed Coke",life_fnc_pickupItem,false,0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "cokeu" && (player distance cursorTarget) < 3 && (life_carryWeight + (["cokeu"] call life_fnc_itemWeight)) <= life_maxWeight ']);
		//Pickup Processed Coke
		life_actions pushBack
		(player addAction["Pickup Processed Coke",life_fnc_pickupItem,true,0,false,false,"",
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "cokep" && (player distance cursorTarget) < 3 && (life_carryWeight + (["cokep"] call life_fnc_itemWeight)) <= life_maxWeight ']);
		...
		' !isNull cursorTarget && (typeOf cursorTarget) isEqualTo "Land_Suitcase_F" && ((cursorTarget getVariable "item") select 0) isEqualTo "BarGate" && (player distance cursorTarget) < 3 ']);	
