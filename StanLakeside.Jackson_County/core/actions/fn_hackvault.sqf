...
if(life_inv_hackingtool isEqualTo 0) exitwith {
...
if (cgbankvault animationPhase "d_l_Anim" isEqualTo 1) exitwith {["The bank appears unlocked...",30,"red"] spawn domsg;};

if (life_inv_hackingtool > 0 && !hacking && cgbankvault animationPhase "d_l_Anim" isEqualTo 0 ) then {
	...
	["Remove",150] call fnc_karma;

	_num = 300; 
	while {_num > 0} do {
		if ( _num isEqualTo 300 || _num isEqualTo 240 || _num isEqualTo 180 || _num isEqualTo 120 || _num isEqualTo 60 ) then {
